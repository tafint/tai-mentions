...
use Test\Mentions\Notification\UserMentionedBlueprint;
...
class UpdateUserMentionsMetadata
{
    ...
    protected function postBecameVisible(Post $post)
    {
        ...
        $link=preg_match('/d\/[0-9]\//', $post->parsedContent, $matches);
        $discid=explode("/", $matches[0]);
        $discussions=Discussion::whereIn('id',array($discid[1]))
            ->get()
            ->first();
        $this->sync($post, array($discussions->start_user_id));
