...
namespace Test\Mentions\Listener;
...
class AddPostMentionedByRelationship
{
    ...
    public function getModelRelationship(GetModelRelationship $event)
    {
        if ($event->isRelationship(Post::class, 'mentionedBy')) {
            return $event->model->belongsToMany(Post::class, 'test_mentions_posts', 'mentions_id', 'post_id', 'mentionedBy');
        }

        if ($event->isRelationship(Post::class, 'mentionsPosts')) {
            return $event->model->belongsToMany(Post::class, 'test_mentions_posts', 'post_id', 'mentions_id', 'mentionsPosts');
        }

        if ($event->isRelationship(Post::class, 'mentionsUsers')) {
            return $event->model->belongsToMany(User::class, 'test_mentions_users', 'post_id', 'mentions_id', 'mentionsUsers');
        }
    }
