...
import AutocompleteDropdown from 'test/mentions/components/AutocompleteDropdown';
    ...
    $textarea
      ...
      .on('click keyup', function(e) {
        ...
        if (mentionStart) {
          ...
          const makeSuggestion = function(title, replacement, content, className = '') {
...
*/
            ...
            if (typed) {
              app.store.all('discussions').forEach(disc=> {
                console.log(disc);
                if (disc.title().toLowerCase().substr(0, typed.length) !== typed) return;
                suggestions.push(
                  makeSuggestion(disc.title(), '[URL=/flarum/d/'+disc.id()+'/]'+disc.title()+'[/URL]',disc.title(), 'MentionsDropdown-user')
                );
              });
            }
          ...
          if (typed) {
            searchTimeout = setTimeout(function() {
              ...
              if (searched.indexOf(typedLower) === -1) {
                app.store.find('discussions', {q: typed, page: {limit: 5}}).then(() => {
